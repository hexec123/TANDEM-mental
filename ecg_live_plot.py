...
class ECGApp:
    ...
    def create_widgets(self):
        tk.Label(self.root, text="Last N seconds:").grid(row=0, column=0)
        self.n_seconds_entry = tk.Entry(self.root)
        self.n_seconds_entry.insert(0, "10")
        self.n_seconds_entry.grid(row=0, column=1)

        self.file_button = tk.Button(self.root, text="Select File", command=self.select_file)
        self.file_button.grid(row=1, column=0, columnspan=2)
        ...
        tk.Label(self.root, textvariable=self.battery_level).grid(row=3, column=0, columnspan=2)
        tk.Label(self.root, textvariable=self.current_hr).grid(row=4, column=0, columnspan=2)
        tk.Label(self.root, textvariable=self.error_message, fg="red").grid(row=6, column=0, columnspan=2)
